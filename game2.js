...
const DISPLAY_MONIATOR = ["Leite ............... R$2,00","Refri-Lata ....... R$6,00","Doces ............. R$2,00"];
...
function createElement(){
  consoleLog(PRODUTO_FASE2[index]);
  let nextItem = document.createElement('img'),
      div_produto = document.getElementById('produtos');
  if(index < 3){
    nextItem.setAttribute('id',''+PRODUTO_FASE2[index]+'');
    nextItem.setAttribute('src','../FatoresHumanos/Image/Produtos/'+PRODUTO_FASE2[index]+'.png');
    nextItem.setAttribute('class','animated slideOutRight');
    nextItem.setAttribute('onclick','getProduto(this.id,'+VALOR_PRODUTO_FASE2[index]+'); addItens('+index+');');
...
function addItens(index){
  let valorItem = document.createElement('li'),
      ...
      texto = document.createTextNode(DISPLAY_MONITOR2[index]),
